    ...
    public sealed partial class ItemPage
    {
        ...
        private static object CreateWorkingSampleContent(string uniqueId)
        {
            return BasicInputSampleFactory.Create(uniqueId)
                ...
                ?? ItemsViewSampleFactory.Create(uniqueId)
                ?? ItemsViewFoundationsSampleFactory.Create(uniqueId)
            ...
            SampleSnippet csharpSnippet)
        {
            ...
            var itemsViewExamples = ItemsViewSampleFactory.CreateExamples(uniqueId);
            if (itemsViewExamples.Count != 0)
            {
                return itemsViewExamples;
            }

            var itemsViewFoundationExamples = ItemsViewFoundationsSampleFactory.CreateExamples(uniqueId);
            if (itemsViewFoundationExamples.Count != 0)
            {
                return itemsViewFoundationExamples;
            }
            ...
            var scrollingExamples = ScrollingSampleFactory.CreateExamples(uniqueId, sampleSnippets);
