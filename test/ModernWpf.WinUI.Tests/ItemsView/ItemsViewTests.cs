...
public class ItemsViewTests
{
    ...
    public void MatchesCurrentWinUiApiDefaultsAndBringIntoViewOptions()
    {
        WpfTestHost.Run(() =>
        {
            ...
            Assert.IsNotNull(Controls.ItemsView.ItemTransitionProviderProperty);
    ...
    public void AppliesDefaultTemplateAndWrapsDataItemsInItemContainers()
    {
        WpfTestHost.Run(() =>
        {
            ...
            itemsView.Padding = new Thickness(3, 4, 5, 6);

            using var host = new TestWindowHost(itemsView, width: 360, height: 240);
            host.UpdateLayout();

            Assert.IsInstanceOfType<StackLayout>(itemsView.Layout);
            Assert.IsFalse(itemsView.IsTabStop);
            Assert.AreEqual(KeyboardNavigationMode.Once, KeyboardNavigation.GetTabNavigation(itemsView));
            Assert.IsNotNull(itemsView.ScrollView);
            Assert.AreEqual(
                KeyboardNavigationMode.Once,
                KeyboardNavigation.GetTabNavigation(itemsView.ScrollView));
            Assert.IsNotNull(itemsView.ItemTemplate);
            var scrollHost = (ItemsRepeaterScrollHost)itemsView.Template.FindName(
                Controls.ItemsView.ScrollHostPartName,
                itemsView);
            var repeater = (ItemsRepeater)itemsView.Template.FindName(
                Controls.ItemsView.ItemsRepeaterPartName,
                itemsView);
            Assert.IsTrue(double.IsNaN(scrollHost.HorizontalAnchorRatio));
            Assert.AreEqual(0.0, repeater.HorizontalCacheLength);
            Assert.AreEqual(itemsView.Padding, repeater.Margin);
    ...
    public void ItemInteractionsFollowSelectionAndInvocationModes()
    {
        WpfTestHost.Run(() =>
        {
            ...
            first.RaiseItemInvoked(ItemContainerInteractionTrigger.PointerReleased, first);
            ...
            first.RaiseItemInvoked(ItemContainerInteractionTrigger.DoubleTap, first);
    ...
    public void BringsItemsIntoViewAndFindsTheViewportItem()
    {
        WpfTestHost.Run(() =>
        {
            ...
            Assert.IsGreaterThan(0.0, itemsView.ScrollView.GetEffectiveOffset().Y);
    ...
    [TestMethod]
    public void ForwardsItemTransitionProviderToTheInnerRepeater()
    {
        WpfTestHost.Run(() =>
        {
    ...
    [TestMethod]
    public void ControlASelectsAllOnlyForMultipleAndExtendedModes()
    {
        WpfTestHost.Run(() =>
        {
            var itemsView = CreateItemsView("Alpha", "Beta", "Gamma", "Delta");
            ...
            {
                itemsView.SelectionMode = selectionMode;
                itemsView.DeselectAll();
                Assert.IsFalse(itemsView.ProcessKeyboardShortcut(Key.A, ModifierKeys.Control));
                Assert.AreEqual(0, itemsView.SelectedItems.Count);
            }
            ...
            })
            {
                itemsView.SelectionMode = selectionMode;
                itemsView.DeselectAll();
                Assert.IsTrue(itemsView.ProcessKeyboardShortcut(Key.A, ModifierKeys.Control));
                Assert.AreEqual(4, itemsView.SelectedItems.Count);
                Assert.IsFalse(itemsView.ProcessKeyboardShortcut(Key.A, ModifierKeys.None));
                Assert.IsFalse(itemsView.ProcessKeyboardShortcut(Key.B, ModifierKeys.Control));
            }
        });
    }

    [TestMethod]
    public void ArrowKeysFollowStackUniformAndLinedFlowGeometry()
    {
        WpfTestHost.Run(() =>
        {
            var linedFlowLayout = new LinedFlowLayout
            {
                LineHeight = 40,
                MinItemSpacing = 8,
                LineSpacing = 8
            };
            linedFlowLayout.ItemsInfoRequested += (_, args) =>
                args.SetDesiredAspectRatios(
                    Enumerable.Repeat(1.5, args.ItemsRangeRequestedLength).ToArray());

            var scenarios = new (Layout Layout, Key Key, string Name)[]
            {
                (new StackLayout(), Key.Down, nameof(StackLayout)),
                (new UniformGridLayout { MinItemWidth = 80, MinItemHeight = 40 }, Key.Right, nameof(UniformGridLayout)),
                (linedFlowLayout, Key.Right, nameof(LinedFlowLayout))
            };

            foreach (var scenario in scenarios)
            {
                var itemsView = CreateItemsView("Alpha", "Beta", "Gamma", "Delta", "Epsilon", "Zeta");
                itemsView.Layout = scenario.Layout;

                using var host = new TestWindowHost(itemsView, width: 420, height: 260);
                host.UpdateLayout();

                KeyEventArgs args = RaisePreviewKey(itemsView, scenario.Key);

                Assert.IsTrue(args.Handled, $"{scenario.Name} should handle {scenario.Key}.");
    ...
    private static KeyEventArgs RaisePreviewKey(UIElement target, Key key)
    {
        var source = PresentationSource.FromVisual(target);
        Assert.IsNotNull(source, "The hosted ItemsView must have a presentation source.");
