...
| `dev\NavigationView\NavigationView.xaml` | 21 | `ModernWpf.Controls\NavigationView\NavigationView.xaml` | Partial | `NavigationViewItemSeparator` horizontal-compact and vertical line states now use `VisualStateEx.Setters`; toggle-pane visible, header, AutoSuggest, closed-compact pane, compact pane-list size, back-button collapsed, pane-collapsed, pane separator visibility, and overflow no-label states also now use setters. The larger NavigationView display-mode, overlay, titlebar, and item-position setter surface remains pending and should continue in small slices; WinUI's `ShadowCaster` pane-collapse/list-size targets are not represented by the current WPF template. |
