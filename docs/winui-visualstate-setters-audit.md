...
| `dev\NavigationView\NavigationView_rs1_themeresources.xaml` | 71 | `ModernWpf.Controls\NavigationView\NavigationView.xaml`, `ModernWpf\Styles\NavigationView.xaml` | Partial | The left-pane `NavigationViewItemPresenter` pointer, disabled, icon-collapsed, chevron-visible, and closed-compact top-level item states now use `VisualStateEx.Setters`. The top-pane `NavigationViewItemPresenter` pointer, disabled, icon-position, chevron-visible, and representable pointer-chevron foreground states now use `VisualStateEx.Setters`; unsupported `AnimatedIcon.State` and missing `InfoBadgePresenter` targets are documented compatibility gaps. Overflow, settings-item, overflow-button, and remaining resource-style states still need separate slices. |
