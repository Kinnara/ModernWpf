...
| `dev\ColorPicker\ColorPicker.xaml` | 20 | `ModernWpf.Controls\ColorPicker\ColorPicker.xaml` | Partial | Top-level spectrum, preview, previous-color, slider, more-button, text-input visibility, and alpha hex-length states now use `VisualStateEx.Setters` and WinUI state names. Orientation layout, color-representation selection, and the nested slider template state setters remain pending because the current WPF template is still structurally simpler than WinUI. |
