...
| `dev\CommandBarFlyout\CommandBarFlyout_themeresources.xaml` | 41 | `ModernWpf.Controls\CommandBarFlyout\CommandBarFlyout.xaml` | Partial | Flyout `AppBarButton` application-view overflow layout, common chrome, and keyboard-accelerator visibility states now use `VisualStateEx.Setters`; flyout `AppBarToggleButton` application-view overflow layout, common/checked/overflow chrome, and keyboard-accelerator visibility states also use setters. Toolbar available-command and combined placement/corner states now use setters while preserving the WPF popup-placement behavior. Input-mode, expansion animation, and shadow setter groups remain pending. |
| `dev\CommonStyles\AppBarButton_themeresources.xaml` | 16 | `ModernWpf.Controls\CommandBar\AppBarButton.xaml` | Partial | Application-view layout, common chrome, keyboard accelerator, and flyout chevron setters that map to existing WPF template parts now use `VisualStateEx.Setters` driven by `AppBarButton`; WinUI's separate `InputModeStates`, `SubItemChevronPanel`, and `OverflowSubItemChevron` targets are not represented by the current template. |
| `dev\CommonStyles\AppBarToggleButton_themeresources.xaml` | 18 | `ModernWpf.Controls\CommandBar\AppBarToggleButton.xaml` | Partial | Application-view layout, common/checked/overflow chrome, check-glyph opacity, and keyboard accelerator setters now use `VisualStateEx.Setters` driven by `AppBarToggleButton`; WinUI's `InputModeStates` touch/game-controller padding and margin setters are not driven by the current WPF implementation. |
...
| `dev\NavigationView\NavigationView_rs1_themeresources.xaml` | 71 | `ModernWpf.Controls\NavigationView\NavigationView.xaml`, `ModernWpf\Styles\NavigationView.xaml` | Partial | The left-pane `NavigationViewItemPresenter` pointer, disabled, icon-collapsed, chevron-visible, and closed-compact top-level item states now use `VisualStateEx.Setters`. The top-pane `NavigationViewItemPresenter` pointer, disabled, icon-position, chevron-visible, and representable pointer-chevron foreground states now use `VisualStateEx.Setters`. The top-pane overflow `NavigationViewItemPresenter` pointer, disabled, content-only, and chevron-visible states now use setters; unsupported `AnimatedIcon.State` and missing `InfoBadgePresenter` targets are documented compatibility gaps. Settings-item, overflow-button, and remaining resource-style states still need separate slices. |
