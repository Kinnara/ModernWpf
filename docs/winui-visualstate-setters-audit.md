...
| `dev\CommandBarFlyout\CommandBarFlyout_themeresources.xaml` | 41 | `ModernWpf.Controls\CommandBarFlyout\CommandBarFlyout.xaml` | Partial | Flyout `AppBarButton` and `AppBarToggleButton` application-view overflow layout states and keyboard-accelerator visibility states now use `VisualStateEx.Setters`. The large common, input-mode, expansion, available-command, placement/corner, and shadow setter matrix remains trigger/storyboard-based and should continue in small slices. |
