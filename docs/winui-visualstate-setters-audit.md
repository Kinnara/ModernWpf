...
| `dev\ColorPicker\ColorSpectrum.xaml` | 5 | `ModernWpf.Controls\ColorPicker\ColorPicker.xaml` | Partial | The WPF-owned box/ring spectrum visibility switch now uses `VisualStateEx.Setters` driven by `ColorSpectrum.Shape`. WinUI pointer, pressed-large, focus, and selection-ellipse color states are not represented by the current WPF input/focus visuals yet. |
