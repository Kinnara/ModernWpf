...
| `dev\CommonStyles\MenuFlyout_themeresources.xaml` | 42 | `ModernWpf.Controls\MenuFlyout\MenuFlyout.xaml`, `ModernWpf\Styles\MenuItem.xaml`, `ModernWpf.Controls\RadioMenuItem\RadioMenuItem.xaml` | Partial | WPF regular submenu items now use `VisualStateEx.Setters` for common chrome, checked opacity, and keyboard-accelerator visibility driven by `MenuItemHelper`; radio menu items already use setters. Submenu headers, placeholder/icon layout states, padding states, reveal states, and WinUI subitem branches remain pending. |
