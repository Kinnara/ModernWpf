...
| `dev\ColorPicker\ColorPicker.xaml` | 20 | `ModernWpf.Controls\ColorPicker\ColorPicker.xaml` | Partial | Top-level spectrum, preview, previous-color, slider, more-button, and text-input visibility states now use `VisualStateEx.Setters` and WinUI state names. Orientation layout, color-representation selection, alpha hex-length, and the nested slider template state setters remain pending because the current WPF template is still structurally simpler than WinUI. |
