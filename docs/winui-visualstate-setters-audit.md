...
| `dev\NavigationView\NavigationView.xaml` | 21 | `ModernWpf.Controls\NavigationView\NavigationView.xaml` | Partial | `NavigationViewItemSeparator` horizontal-compact and vertical line states now use `VisualStateEx.Setters`; header, AutoSuggest, closed-compact pane, back-button collapsed, pane-collapsed, and pane separator visibility states also now use setters. The larger NavigationView display-mode, pane/list-size, overlay, titlebar, item-position, and overflow setter surface remains pending and should continue in small slices; WinUI's `ShadowCaster` pane-collapse target is not represented by the current WPF template. |
