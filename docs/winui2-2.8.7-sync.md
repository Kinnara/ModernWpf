...
| NavigationView | Pending | Ported APITest slices | Existing WPF port; upstream defaults/basic setters, value coercion, pane-property API checks, selected-item clearing, navigation item UIA type, expand/collapse pattern availability, settings item tag/tooltip, and footer-item no-crash checks are ported. Top mode layout, pane launch interplay, container mapping, hierarchy, visual-tree, and resource tests remain pending. |
