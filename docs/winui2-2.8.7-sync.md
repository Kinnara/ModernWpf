...
| Repeater / ItemsRepeater layouts | Pending | Ported initial APITest slices | Existing WPF port; upstream `IndexPath`, `ItemsSourceView`, `RecyclePool`, and initial `SelectionModel` API/event/range/nested coverage is ported. Layout, selection mutation, viewport, phasing, focus, data-source integration, and visual tests remain pending. |
