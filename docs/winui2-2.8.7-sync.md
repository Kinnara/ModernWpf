...
| Repeater / ItemsRepeater layouts | Pending | Ported initial APITest slices | Existing WPF port; upstream `IndexPath`, `ItemsSourceView`, `RecyclePool`, and flat `SelectionModel` API/event/range coverage is ported. Layout, nested selection, viewport, phasing, focus, data-source integration, and visual tests remain pending. |
