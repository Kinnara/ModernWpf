...
| Repeater / ItemsRepeater layouts | Pending | Ported initial APITest slices | Existing WPF port; upstream `IndexPath`, `ItemsSourceView`, `RecyclePool`, and broad `SelectionModel` API/event/range/nested/mutation/property/regression/children-requested coverage is ported. Layout, advanced selection helpers, viewport, phasing, focus, data-source integration, and visual tests remain pending. |
