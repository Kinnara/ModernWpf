...
| ProgressBar resources | Pending | Ported APITests | Upstream `ProgressBarTrackHeight` resource overridability test is ported; broader style/resource parity remains pending. |
...
| Repeater / ItemsRepeater layouts | Pending | Ported initial APITest slices | Existing WPF port; upstream `IndexPath`, `ItemsSourceView`, `RecyclePool`, broad `SelectionModel` API/event/range/nested/mutation/property/regression/children-requested coverage, direct `ItemTemplate` / `RecyclingElementFactory` API coverage, and direct `ElementAnimator` API coverage are ported. WinRT-only `ICustomPropertyProvider` selection metadata is excluded; layout, advanced selection helpers, viewport, phasing, focus, data-source integration, and visual tests remain pending. |
