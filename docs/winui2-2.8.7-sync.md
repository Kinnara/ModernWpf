...
| NavigationView | Pending | Ported initial APITest slice | Existing WPF port; upstream defaults/basic setters, value coercion, and pane-property API checks are ported. Top mode, selection, pane launch interplay, automation peers, tooltips, container mapping, hierarchy, and resource tests remain pending. |
