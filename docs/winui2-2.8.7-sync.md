...
| Repeater / ItemsRepeater layouts | Pending | Ported initial APITest slices | Existing WPF port; upstream `IndexPath`, `ItemsSourceView`, `RecyclePool`, broad `SelectionModel` API/event/range/nested/mutation/property/regression/children-requested coverage, and direct `ItemTemplate` / `RecyclingElementFactory` API coverage are ported. WinRT-only `ICustomPropertyProvider` selection metadata is excluded; layout, advanced selection helpers, viewport, phasing, focus, data-source integration, and visual tests remain pending. |
