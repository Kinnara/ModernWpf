...
| NavigationView | Pending | Ported APITest slices | Existing WPF port; upstream defaults/basic setters, value coercion, pane-property API checks, selected-item clearing, navigation item UIA type, expand/collapse pattern availability, settings item tag/tooltip, footer-item no-crash, menu item/container mapping, top-mode collection clearing, top-mode hierarchy, item tooltip, and item-outlives-parent checks are ported. Top mode layout, pane launch interplay, visual-tree, and resource tests remain pending. |
