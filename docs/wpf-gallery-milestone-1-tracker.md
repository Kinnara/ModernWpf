...
- `dotnet test test\ModernWpf.Gallery.Tests\ModernWpf.Gallery.Tests.csproj --configuration Release --filter "BasicInputControlExamplesMatchOfficialWpfGallerySampleCode|CollectionsControlExamplesMatchOfficialWpfGallerySampleCode|LayoutControlExamplesMatchOfficialWpfGallerySampleCode|DateMediaAndStatusControlExamplesMatchOfficialWpfGallerySampleCode" -p:UseSharedCompilation=false`
...
| Sample code panes | Partial | Basic Input, Collections, Date & Calendar, Layout, Media, and Status & Info `ControlExample` snippets are now covered by runtime parity tests against the official WPF Gallery reference values, including the ResizeGrip C# pane; continue section-by-section coverage for remaining WPF-equivalent pages and C# snippets where present. |
...
| Design Guidance section | Partial | Partial | Open | Color, Iconography, Typography, Spacing, and Geometry now use adapted official WPF Gallery page shells/XAML; section visual audit remains. |
...
| Basic Input section | Partial | Partial | Open | Basic Input section header, navigation cards, `ViewModel.*` bindings, item pages, and item-page sample code panes now use adapted official WPF Gallery resources; section page visual audit remains. |
| Button | Done | Done | Open | Adapted from official WPF Gallery XAML with `PageHeader` and `ControlExample`; sample headers/snippets are runtime-covered; screenshot parity remains. |
| CheckBox | Done | Done | Open | Adapted from official WPF Gallery XAML with `PageHeader` and `ControlExample`; sample headers/snippets are runtime-covered; screenshot parity remains. |
