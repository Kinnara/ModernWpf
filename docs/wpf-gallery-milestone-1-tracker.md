...
| All controls page | Partial | Header, navigation cards, and `ViewModel.PageTitle` / `ViewModel.NavigationCards` bindings now use adapted official structure; verify grouping, sort order, card subtitles, tile sizing, and search behavior against official WPF Gallery. |
...
| All controls | Partial | Partial | Open | Runtime shell checks plus adapted `PageHeader`, navigation-card resources, and official-style `ViewModel.*` bindings exist; search/sort and exact visual audit still needed. |
