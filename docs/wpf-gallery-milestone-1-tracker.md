...
| Page shell and header model | Mostly done | Item, section, support, sample chrome, Home header tile strip, navigation-card resources, and category/all-controls headers have been moved toward WPF Gallery structure and spacing. |
...
| Page-by-page exact XAML audit | Open | For each WPF-equivalent page, compare ModernWpf sample factories against official WPF Gallery XAML and replace approximations with adapted copies where feasible. Color section controls, User Dashboard, All controls search/sort behavior, remaining section page content, and visual parity remain open. |
| Home page | Partial | Header tile strip and navigation cards now use adapted official `TileGallery`/`HeaderTile` and `NavigationCardTemplate`; compare remaining first-viewport screenshot parity against official WPF Gallery. |
| All controls page | Partial | Header and navigation cards now use adapted official `PageHeader` and `NavigationCardTemplate`; verify grouping, sort order, card subtitles, tile sizing, and search behavior against official WPF Gallery. |
| Section pages | Partial | Headers and navigation cards now use adapted official `PageHeader` and `NavigationCardTemplate`; verify each section's title, description, hero/card layout, item order, and empty-space behavior against official WPF Gallery. |
...
| --- | --- | --- | --- | --- |
| Home | Partial | Partial | Open | Header tile strip and navigation cards now use adapted official WPF Gallery `TileGallery`/`HeaderTile` and `NavigationCardTemplate`; screenshot parity still needs audit. |
| All controls | Partial | Partial | Open | Runtime shell checks plus adapted `PageHeader` and navigation-card resources exist; search/sort and exact visual audit still needed. |
...
| Basic Input section | Partial | Partial | Open | Basic Input section header, navigation cards, and item pages now use adapted official WPF Gallery resources; section page visual audit remains. |
