...
| Recommended resource entry | ModernWpf compatibility contract | `<ui:ThemeResources />` plus `<ui:FluentControlsResources />` | README recommends the new entry, and `ModernWpf.Theme.Tests` now exercises the entry on net8 and net10. Gallery still uses `<ui:XamlControlsResources />`. | Keep focused net10 smoke coverage for the recommended entry and keep legacy entry behavior unchanged. |
