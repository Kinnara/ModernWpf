...
- RichTextEdit remains open as recorder-input coverage, not as a closed visual
  proof. Current external runs still report `AfterOutput=""`; TextBox success
  does not prove RichTextBox recording because TextBox uses the recorder's
  writable `ValuePattern` path while RichTextBox exposes `TextPattern` without a
  writable UIA value. The runtime `RichTextEditAcceptsTextCompositionInput`
  test remains the product proof for WPF text composition.
