...
- WinUI root `Grid` chrome is represented by an inert WPF `BorderEx` layer, and WinUI `SwitchAreaGrid.CornerRadius` is represented by the WPF `Border` used for `SwitchAreaGrid`.
