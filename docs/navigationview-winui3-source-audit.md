...
| Source template uses `RootSplitView`, left/top menu hosts, pane/header/footer presenters, top overflow button, `ShadowCaster`, visual-state groups, and NavigationViewItemPresenter slots. | `NavigationView.xaml` and `Styles\NavigationView.xaml` expose the matching named parts and use `VisualStateEx.Setters` for WPF-compatible source setter blocks, including the source compact-list, pane-collapsed, and pane-overlay shadow-caster targets. `ShadowCaster` is a WPF `ThemeShadowChrome` template part so it can keep those state targets while rendering a source-depth shadow. |
| Source clamps `TemplateSettings.OpenPaneLength` to the current control width and binds `RootSplitView.OpenPaneLength` / `ShadowCaster.Width` to that template setting. | ModernWpf now exposes `NavigationViewTemplateSettings.OpenPaneLength`, updates it from size and `OpenPaneLength` changes, and binds both the WPF `SplitView` and `ShadowCaster` template part to the source template-setting path. |
| Source applies `ThemeShadow{}` to `ShadowCaster` and sets `Translation.Z` from `PaneOverlayShadowDepth`, default `16`. | ModernWpf defines `PaneOverlayShadowDepth=16` and maps the `ShadowCaster` template part to `ThemeShadowChrome.Depth`, using the shared WinUI recipe renderer as the WPF substitute for the compositor shadow. |
...
- WinUI keyboard accelerators, access keys, gamepad focus, composition header animations, and raw Axe/TestUI automation remain documented platform gaps. The `ShadowCaster` template part and WPF `TranslateTransform` keep source state targets and pane-overlay transitions represented; the actual WinUI compositor `ThemeShadow` is represented by ModernWpf's shared software `ThemeShadowChrome` renderer rather than a compositor visual.
