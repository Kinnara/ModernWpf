...
- WinUI keyboard accelerators, access keys, gamepad focus, composition header animations, ThemeShadow, and raw Axe/TestUI automation remain documented platform gaps. The `ShadowCaster` template part and WPF `TranslateTransform` keep source state targets and pane-overlay transitions represented; the actual WinUI `ThemeShadow` compositor effect remains a platform gap.
