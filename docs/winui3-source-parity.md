...
| --- | --- | --- | --- | --- |
| ToggleSwitch | `src\dxaml\xcp\dxaml\lib\ToggleSwitch_Partial.cpp` | `ModernWpf.Controls\ToggleSwitch\ToggleSwitch.cs`, `test\ModernWpf.WinUI.Tests\ToggleSwitch\ToggleSwitchApiTests.cs` | Guessed implementation; must be replaced/adapted from WinUI source | Existing ModernWpf ToggleSwitch is still a guessed implementation. The prior work only aligned one drag-threshold behavior and added tests for that behavior; it did not replace the control implementation with the WinUI source shape. Treat the whole control as pending source-parity replacement. |
