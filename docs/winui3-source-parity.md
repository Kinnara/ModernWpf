...
| AppBarButton / AppBarToggleButton | `src\dxaml\xcp\dxaml\lib\AppBarButton_Partial.cpp`, `src\dxaml\xcp\dxaml\lib\AppBarButtonHelpers.h`, `src\controls\dev\CommonStyles\AppBarButton_themeresources.xaml` | `ModernWpf.Controls\CommandBar\AppBarButton.cs`, `ModernWpf.Controls\CommandBar\AppBarToggleButton.cs`, `ModernWpf.Controls\CommandBar\AppBarButton.xaml`, `ModernWpf.Controls\CommandBar\AppBarToggleButton.xaml`, `ModernWpf.Controls\CommandBarFlyout\CommandBarFlyout.xaml` | Guessed implementation; source parity pending | These controls are widely reused by `CommandBar`, `CommandBarFlyout`, and flyout templates. Current code still carries old ModernWpf/WPF `ToolBar`-driven assumptions for normal `CommandBar` overflow and should be audited against WinUI 3 before treating CommandBar-family parity as high confidence. |
