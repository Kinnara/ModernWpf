...
`LayoutCompatibilityApiTests.SourceBackedShadowTemplatesRenderVisibleShadowPixels` renders representative source-backed template chromes, covering `FlyoutPresenter` at source depth `32` / medium windowed-popup insets, the compact `NumberBox` popup at source depth `16`, `AutoSuggestBox` suggestions, `CommandBar` overflow, and `TeachingTip` content-root shadow chrome. The guard hides each caster child during measurement so it isolates the WPF shadow layer, then verifies that visible shadow pixels extend beyond the arranged caster bounds and keep a downward centroid like WinUI ThemeShadow.
