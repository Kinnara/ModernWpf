...
The source-shaped live WinUI capture tool uses those staged `.mask.txt` sidecars to render each manifest geometry through a real WinUI `ThemeShadow` and `RenderTargetBitmap` path:

```powershell
dotnet build .\tools\theme-shadow\WinUIReferenceCapture\WinUIThemeShadowCapture.csproj -r win-x64 -p:UseSharedCompilation=false
.\tools\theme-shadow\WinUIReferenceCapture\bin\Debug\net8.0-windows10.0.19041.0\win-x64\WinUIThemeShadowCapture.exe --reference-dir $referenceDir
...
The capture app reads `docs/theme-shadow-reference-captures.json`, renders light-theme white casters with source depths (`16`, `32`, or `128`) into the manifest canvas sizes, and writes the canonical `*-shadow-only.png` files. Pass `--target <name-or-referenceFileBase>` to regenerate one target. This is a live WinUI ThemeShadow raster capture, but it is still source-shaped geometry rather than a full actual-control WinUI template capture; use the comparison result to drive the remaining raster/template tuning instead of treating the generated folder as final parity proof.

The canonical WinUI capture checklist is `docs/theme-shadow-reference-captures.json`. It pins the nine shadow-only reference filenames, canvas sizes, owning ModernWpf rendered-template tests, WinUI source evidence, the capture project, and the `.mask.txt` sidecar convention for references that include the source caster/content. `TemplateParityTests.ThemeShadowReferenceCaptureManifestCoversRenderedSnapshotTargets` guards that manifest so a new source-backed shadow host cannot leave the WinUI capture contract implicit.

`TemplateParityTests.ThemeShadowReferenceDirectoryCoversCaptureManifestWhenOptedIn` also honors `MODERNWPF_SHADOW_REFERENCE_DIR`. When that env var is set, the test requires every manifest target to have a same-named `.png` or `.txt` file and verifies each reference dimension against the manifest. PNG references must also carry a matching `.mask.txt` sidecar with the expected target name, `shadow-only` kind, canvas size, and in-bounds `IgnoredBounds`; after clearing that ignored caster/content rectangle, the guard requires visible darkened shadow pixels outside the mask. Run it before the rendered-template comparison filter to catch incomplete, wrong-sized, unmasked, or blank WinUI capture folders:
