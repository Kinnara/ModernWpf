...
The same capture executable also accepts `--capture-mode actual-control` for the smoke-proven actual-control targets:

```powershell
.\tools\theme-shadow\WinUIReferenceCapture\bin\x64\Debug\net8.0-windows10.0.19041.0\WinUIThemeShadowCapture.exe --reference-dir $actualControlReferenceDir --capture-mode actual-control --target FlyoutPresenter --target CommandBar-overflow-popup-shadow-only --target MenuFlyoutPresenter --target ContentDialog-background-shadow-shadow-only
```

That path captures real WinUI `FlyoutPresenter` and `MenuFlyoutPresenter` controls directly, and extracts the real WinUI `CommandBar` `SecondaryItemsControlShadowWrapper` and `ContentDialog` `BackgroundElement` template parts after layout into the existing manifest canvas/mask contract. It is not yet the full nine-target actual-control reference set; `NumberBox`, `AutoSuggestBox`, `CommandBarFlyout`, `TeachingTip`, and `NavigationView` actual-control captures still need a resource-complete WinUI template path or equivalent live-template proof.
...
This is still a WPF substitution, not a literal WinUI compositor port. The depth, blur, offset, inset, hollow-center behavior, and light/dark opacity constants now come from WinUI source and WinUI masters, and representative ModernWpf source-backed templates now have rendered-pixel guards for childful and childless chrome shapes. The `ThemeShadowDropShadowSystemThemeRedrawRTB` source PNG masters can now be read directly from a local WinUI checkout for the core `50x50` depth-32 canvas. The source-geometry live WinUI capture app can now generate all nine manifest PNGs from real WinUI `ThemeShadow` rendering, writes provenance sidecars for those captures, and the `-RequireReferencePngs` plus reference-directory guards pass against that folder. The same tool now has smoke-proven actual-control captures for `FlyoutPresenter`, `CommandBar` overflow, `MenuFlyoutPresenter`, and `ContentDialog` background, but the goal is still open because actual-control WinUI template captures are not proven for all nine targets and the rendered-template comparison currently exposes a material childful mismatch: the childless comparison passes, but the childful comparison fails first on `FlyoutPresenter-shadow-only` with the ModernWpf centroid shifted right/down relative to the live WinUI reference. The next parity round should continue the remaining five actual-control captures or use the existing actual-control/source-geometry comparison evidence to drive targeted tuning.
