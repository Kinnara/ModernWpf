...
This is still a WPF substitution, not a literal WinUI compositor port. The depth, blur, offset, inset, hollow-center behavior, and light/dark opacity constants now come from WinUI source and WinUI masters, and representative ModernWpf source-backed templates now have rendered-pixel guards for childful and childless chrome shapes. The `ThemeShadowDropShadowSystemThemeRedrawRTB` source PNG masters can now be read directly from a local WinUI checkout for the core `50x50` depth-32 canvas. The live WinUI capture app can now generate all nine manifest PNGs from real WinUI `ThemeShadow` rendering in both source-geometry mode and smoke-proven actual-control mode, writes provenance sidecars for those captures, and the `-RequireReferencePngs` plus reference-directory guards pass against the refreshed actual-control folder. The goal is still open because the rendered-template comparison currently exposes a material childful mismatch: the childless comparison passes, but the childful comparison fails first on `FlyoutPresenter-shadow-only` with the ModernWpf centroid shifted right/down relative to the live WinUI reference (`Reference=(58.404,72.923)`, `Actual=(67.849,81.624)`, X tolerance `8`). The next parity round should tune or explain that rendered-template raster/layout mismatch before treating the actual-control references as parity proof.
