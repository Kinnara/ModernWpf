...
The same capture executable also accepts `--capture-mode actual-control` for the direct presenter prototype targets:

```powershell
.\tools\theme-shadow\WinUIReferenceCapture\bin\x64\Debug\net8.0-windows10.0.19041.0\WinUIThemeShadowCapture.exe --reference-dir $actualControlReferenceDir --capture-mode actual-control --target FlyoutPresenter --target MenuFlyoutPresenter
```

That prototype captures real WinUI `FlyoutPresenter` and `MenuFlyoutPresenter` controls into the existing manifest canvas/mask contract. It is not yet the full nine-target actual-control reference set.
