...
`LayoutCompatibilityApiTests.SourceBackedShadowTemplatesRenderVisibleShadowPixels` renders representative source-backed template chromes, currently `FlyoutPresenter` at source depth `32` / medium windowed-popup insets and the compact `NumberBox` popup at source depth `16`. The guard hides each caster child during measurement so it isolates the WPF shadow layer, then verifies that visible shadow pixels extend beyond the arranged caster bounds and keep a downward centroid like WinUI ThemeShadow.
