...
Static ModernWpf window captures use `PrintWindow` first and fall back to an activated screen-rect capture when `PrintWindow` returns a blank image. Installed WinUI Gallery reference captures prefer activated screen capture because `PrintWindow` often returns compositor backdrop instead of XAML content for that app. If a capture returns a nonblank but invalid reference surface, such as a desktop/wallpaper or Mica backdrop crop from a WinUI composition window, the harness rejects the result when the primary crop has very low visible luminance variation and fails the run instead of reporting false parity.
...
- Dark: `artifacts/visual-checks/20260513-030925/report.md`
- Light: `artifacts/visual-checks/20260513-031029/report.md`
...
- `NavigationView` header parity is fixed in the current branch. Manual inspection found the valid WinUI reference crop in `artifacts/visual-checks/20260513-030925/NavigationView/winui3-NavigationView-primary-crop.png` showing `Sample Page 1`, while the ModernWpf crop from that run still showed `This is Header Text`. The sample now selects the first item after the `SelectionChanged` handler is attached, matching the WinUI Gallery source behavior. Current ModernWpf-only verification shows `Sample Page 1` in Dark (`artifacts/visual-checks/20260513-034332/report.md`) and Light (`artifacts/visual-checks/20260513-034353/report.md`).
- `TeachingTip` expand animation now uses WinUI's final 2.8.7 minimum start scale expression, and ModernWpf interaction verification passes through the in-app rendered open-content artifact: Dark `artifacts/visual-checks/20260513-025705/report.md`, Light `artifacts/visual-checks/20260513-025743/report.md`.
- The current local execution context cannot always produce reliable installed WinUI Gallery XAML pixels through GDI screen capture, `PrintWindow`, DWM thumbnail capture, or Windows Graphics Capture experiments; these paths can return backdrop/wallpaper or black client content instead of XAML. The harness now fails invalid installed-reference captures instead of reporting false parity. Example rejected reports: `artifacts/visual-checks/20260513-023339/report.md` and `artifacts/visual-checks/20260513-032310/report.md`.
